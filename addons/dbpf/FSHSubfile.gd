...
func load(file, dbdf=null):
    ...
    self.file_size = self._get_int_from_bytes(raw_data.slice(ind, ind+4))
    ind += 4
    # 4 bytes (uint32) - number of entries
    var num_entr : int =  self._get_int_from_bytes(raw_data.slice(ind, ind+4))
    ind += 4
    # 4 bytes (char) - directory ID 
    var dir_id = raw_data.slice(ind, ind+4).get_string_from_ascii()
    ind += 4
    
    # Directory
    var dir_bytes = raw_data.slice(ind, ind+8)
    var directory = []
        
    while len(directory) < num_entr:
        # 4 bytes (char) - entry tag  // e.g. "br02"
        var tag = dir_bytes.slice(0, 4).get_string_from_ascii()
        ind += 4
        # 4 bytes (uint32) - entry offset 
        var offset : int =  self._get_int_from_bytes(dir_bytes.slice(4, 8))
        ind += 4
        directory.append(FSH_Entry.new(tag, offset))
        dir_bytes = raw_data.slice(ind, ind+8)
    ...
    for entry in directory:
        ...
        entry.block_size = self._get_int_from_bytes(raw_data.slice(ind, ind+3))
        ind += 3
        # 2 bytes (uint16) - image width
        entry.width = self._get_int_from_bytes(raw_data.slice(ind, ind+2))
        ind += 2
        # 2 bytes (uint16) - image height
        entry.height = self._get_int_from_bytes(raw_data.slice(ind, ind+2))
        ind += 2
        # 2 bytes (uint16) - X axis coordinate (Center X)
        entry.x_coord = self._get_int_from_bytes(raw_data.slice(ind, ind+2))
        ind += 2
        # 2 bytes (uint16) - Y axis coordinate (Center Y)
        entry.y_coord = self._get_int_from_bytes(raw_data.slice(ind, ind+2))
        ind += 2
        # 2 bytes - X axis position (Left X pos.)[uint12] + internal flag [uint1] + unknown [uint3]
        entry.x_pos = self._get_int_from_bytes(raw_data.slice(ind, ind+2)) >> 2
        ind += 2
        # 2 bytes - Y axis position (Top Y pos.)[uint12] + levels count (mipmaps) [uint4]
        entry.y_pos = self._get_int_from_bytes(raw_data.slice(ind, ind+2)) >> 2
        entry.mipmaps = 3&self._get_int_from_bytes(raw_data.slice(ind, ind+2))
        ind += 2
    # x bytes - image data 
